...
| 配置名 | 业务环境变量 | 系统凭据引用 |
| --- | --- | --- |
| default | `ZENMUX_API_KEY` | `grok-designer/zenmux/default` |
...
```bash
node "$SKILL_DIR/scripts/credential-ui/src/profile.ts" run default -- python3 "$SKILL_DIR/scripts/grok-designer" ui "评审页面" -f "/path/to/page.html" -o review.md
```
